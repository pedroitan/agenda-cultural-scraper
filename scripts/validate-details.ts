...
async function validateDetails() {
  const { data: events, error } = await supabase
    .from('events')
    .select('source, description, organizer, performers, duration, age_restriction')
    .eq('source', 'sympla')
    .limit(20)
  ...
  console.log('=== Sympla Events Detail Validation ===\n')
  ...
  events.forEach((ev, i) => {
    ...
    console.log(`${i + 1}. Description: ${ev.description ? '✓' : '✗'} (${ev.description?.slice(0, 50) || 'empty'})`)
