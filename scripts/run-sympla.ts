...
async function main() {
  ...
  try {
    console.log(`\n========== Starting Sympla scraper for ${city} ==========\n`)
    ...
    console.log(JSON.stringify({ level: 'info', msg: 'scrape_success', source: 'sympla', runId: run.id, ...metrics }))
  } catch (err) {
    ...
    console.error(JSON.stringify({ level: 'error', msg: 'scrape_failed', source: 'sympla', runId: run.id, error: message, details: errorDetails }))
    throw err
  }
}
