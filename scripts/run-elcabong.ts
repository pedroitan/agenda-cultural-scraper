...
async function main() {
  ...
  try {
    console.log(`\n========== Starting El Cabong scraper for ${city} ==========\n`)
    const result = await runElCabongScrape(input)
    ...
    console.log(JSON.stringify({ level: 'info', msg: 'scrape_success', source: 'elcabong', runId: run.id, ...metrics }))
  } catch (err) {
    ...
    console.error(JSON.stringify({ level: 'error', msg: 'scrape_failed', source: 'elcabong', runId: run.id, error: message, details: errorDetails }))
    throw err
  }
}
